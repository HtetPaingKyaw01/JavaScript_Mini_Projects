...
function population(){
    ...
    for(let i = 0;i < countries_data.length;i++){
        ...
        for(country of e){
            if(country.population > a){
                a = country.population;
            };
        };

        let c = e.filter( el => el.population == a);
        for(let j = 0;j < c.length;j++){
            b.push({country : c[j].name, population : c[j].population});
        };

        let d = e.filter( el => el.population !== a);

        e = d;
    };

    let aaa = b.slice(0,10);
...
function languages(){
    let a = [];
    for(country of countries_data){
        a.push({country : country.name, languages : country.languages});
    };

    let b = [];
    for(country of countries_data){
        for(let i = 0;i < country.languages.length;i++){
            b.push(country.languages[i]);
        };
    };
    ...
    for(let i = 0;i < es.length;i++){
        ...
        let gs = es.filter( el => el.length == f);
        
        
        for(g of gs){
            h.push({name : g[0], length : g.length});
        };

        let l = es.filter(el => el.length !== f);
        es = l;
    };
    let aaa = h.slice(0,10);
...
for(ya of y){
    ...
    aa.style.justifyContent = 'space-between';
    aa.style.alignItems = 'center';
    aa.style.marginBottom = '10px';
    bb.innerText = ya.name;
    bb.style.width = '82px';
    cc.style.backgroundColor = '#ffa500';
    cc.style.width = `${ya.length}%`;
    cc.style.height = '30px';
    dd.innerText = ya.length;
    dd.style.width = '18px';
...
for(xa of x){
    ...
    let ee = document.createElement('div');
    let dd = document.createElement('span');
    let oo = ((xa.population / 10000000) / 2);
    aa.style.display = 'flex';
    aa.style.justifyContent = 'space-between';
    aa.style.alignItems = 'center';
    aa.style.marginBottom = '10px';
    bb.innerText = xa.country;
    bb.style.width = '82px';
    cc.style.backgroundColor = '#ffa500';
    cc.style.width = `${oo}%`;
    cc.style.height = '30px';
    dd.innerText = xa.population;
    dd.style.width = '18px';
    ee.style.width = '500px';
    aa.appendChild(bb);
    ee.appendChild(cc);
    aa.appendChild(ee);
    aa.appendChild(dd);
    outputB.appendChild(aa);
};

outputBox.appendChild(output);
let btnLeft = document.querySelector('.btn-left');
let btnRight = document.querySelector('.btn-right');
let btnBoxText = document.querySelector('.btn-box-text');

btnLeft.addEventListener('click', el=> {
    outputB.classList.remove('hide');
    outputB.classList.add('show');
    output.classList.remove('show');
    output.classList.add('hide');
    btnBoxText.innerText = '10 Most populated countries in the world';
});

btnRight.addEventListener('click', el => {
    outputB.classList.remove('show');
    outputB.classList.add('hide');
